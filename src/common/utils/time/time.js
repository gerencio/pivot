...
function getBestGranularityDuration(timeRange) {
...
exports.getBestGranularityDuration = getBestGranularityDuration;
function getTickDuration(timeRange) {
    var len = timeRange.end.valueOf() - timeRange.start.valueOf();
    if (len > 95 * chronoshift_1.day.canonicalLength) {
        return chronoshift_1.Duration.fromJS('P1M');
    }
    else if (len > 20 * chronoshift_1.day.canonicalLength) {
        return chronoshift_1.Duration.fromJS('P1W');
    }
    else if (len > 6 * chronoshift_1.day.canonicalLength) {
        return chronoshift_1.Duration.fromJS('P1D');
    }
    else if (len > 2 * chronoshift_1.day.canonicalLength) {
        return chronoshift_1.Duration.fromJS('PT12H');
    }
    else if (len > 23 * chronoshift_1.hour.canonicalLength) {
        return chronoshift_1.Duration.fromJS('PT6H');
    }
    else if (len > 3 * chronoshift_1.hour.canonicalLength) {
        return chronoshift_1.Duration.fromJS('PT1H');
    }
    else if (len > 30 * chronoshift_1.minute.canonicalLength) {
        return chronoshift_1.Duration.fromJS('PT5M');
    }
    else {
        return chronoshift_1.Duration.fromJS('PT1M');
...
function getTimeTicks(timeRange, timezone) {
    var start = timeRange.start, end = timeRange.end;
    var tickDuration = getTickDuration(timeRange);
